...
def paint_piece_xy(bw, pos):
    ...
    if (images['use_pieces']):
        # correction for images. (drawing circles needs the center of the circle, blitting the upper left)
        r = images['piece_white'].get_width()/2
        x = (x-r)
        y = (y-r)
        
        if (bw):
            screen.blit(images['piece_black'], (x,y))
        else:
            screen.blit(images['piece_white'], (x,y))
    else:
        if bw:
...
def paint_dice(avail, used=[]):
    ...
    for (num, color) in dice:
        ...
        if (images['use_dice']):
            screen.blit(images['die%s-%s' % (color, num)], rect)
        else:
...
def load_images():
    ...
    if (os.path.exists("board.png")):
        images['use_board'] = True
        raw_board = pygame.image.load("board.png")
        scaled_board = pygame.transform.smoothscale(raw_board, (w/15*13, h))
        images['board'] = scaled_board
    else:
        images['use_board'] = False
    ...
    if (os.path.exists("Dice")):
        ...
        for i in range(1,7):
            images['diegreen-%s' % i] = pygame.transform.smoothscale(
                ...
                (w/15, w/15))
            images['diered-%s' % i]   = pygame.transform.smoothscale(
                pygame.image.load(os.path.join('Dice', 'dieRed%s.png' % i)),
                (w/15, w/15))
    else:
        images['use_dice'] = False

    if (os.path.exists("pieces")):
        images['use_pieces'] = True
        raw_white = pygame.image.load(os.path.join('pieces', 'piece_white.png'))
        images['piece_white'] = pygame.transform.smoothscale(raw_white, (w/15, w/15))
        raw_black = pygame.image.load(os.path.join('pieces', 'piece_black.png'))
        images['piece_black'] = pygame.transform.smoothscale(raw_black, (w/15, w/15))
    else:
        images['use_pieces'] = False
